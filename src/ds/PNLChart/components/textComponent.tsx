...
export const textComponent = (isCall: boolean, isLong: boolean) => {
  ...
  const leftText = isCall ? (isLong ? 'Worthless exp.' : 'Limited profit') : isLong ? 'Unlimited profit' : 'Unlimited downside';

  const rightText = isCall ? (isLong ? 'Unlimited upside' : 'Unlimited downside') : isLong ? 'Worthless exp.' : 'Limited profit';
